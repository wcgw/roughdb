...
///
/// Typical usage:
/// 1. Construct `TableBuilder::new(file, block_size, restart_interval)`.
...
impl TableBuilder {
  ...
  pub(crate) fn new(file: File, block_size: usize, restart_interval: usize) -> Self {
    Self {
  ...
  /// Complete the SSTable: flush any remaining data, write the index block,
  /// an empty metaindex block, and the footer.  Returns the total file size.
  pub(crate) fn finish(mut self) -> Result<u64, Error> {
    ...
    // Write empty metaindex block (no filter blocks in Phase 3).
    let metaindex_handle = {
      let mut empty_meta = BlockBuilder::new(1);
      let meta_data = empty_meta.finish().to_vec();
      write_raw_block(&mut self.dest, &meta_data, self.offset)?
    };
    self.offset += metaindex_handle.size + 5; // data + 5-byte trailer
...
mod tests {
  ...
  fn empty_table_finish() {
    let tmp = tempfile::NamedTempFile::new().unwrap();
    let file = tmp.reopen().unwrap();
    let builder = TableBuilder::new(file, 4096, 16);
    let size = builder.finish().unwrap();
