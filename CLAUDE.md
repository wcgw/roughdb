...
- **`Status`**: Result type carrying an error code (`NotFound`, `Corruption`, `IOError`, `InvalidArgument`, `NotSupported`) and an optional message. Replace current `Option<Vec<u8>>` returns from `Db`. See `include/leveldb/status.h`.
